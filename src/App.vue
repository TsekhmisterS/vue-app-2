...
export default {
  ...
  methods: {
    addTicker(tickerValue) {
      const newTicker = { name: tickerValue, price: "loading" };
      this.tickersArray.push(newTicker);
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=a8c6d0fb0f3c0cc3f7b35ae90f9d15f67c984bcdc1b34af679c16232256a2283`
        );

        const data = await f.json();
        this.tickersArray.find((t) => t.name === newTicker.name).price =
    ...
    removeTicker(tickerValue) {
      this.tickersArray = this.tickersArray.filter(
        (item) => item.name !== tickerValue.name
      );
    },

    normalizeGraphValues() {
      const maxValue = Math.max(...this.graph);
      const minValue = Math.min(...this.graph);

      return this.graph.map(
        (pr) => 5 + ((pr - minValue) * 95) / (maxValue - minValue)
      );
    },

    selectTicker(selectedTicker) {
      this.selectedElement = selectedTicker;
      this.graph = [];
    },
  },
};
...
<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
      ...
      <hr
        v-if="tickersArray.length"
        class="w-full border-t border-gray-600 my-4"
      />
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div
          v-for="item in tickersArray"
          :key="item.name"
          @click="selectTicker(item)"
          :class="{ 'border-4': selectedElement === item }"
      ...
      <section v-if="selectedElement" class="relative">
        ...
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
            v-for="(col, ind) in normalizeGraphValues()"
            :key="ind"
            :style="{ height: `${col}%` }"
